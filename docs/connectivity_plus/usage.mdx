...
### Proxy settings [Android and iOS only]

Using Connectivity API, you can get the proxy settings from the OS

``` dart
ProxyResult proxyResult = await _connectivity.getProxySettings();

if (proxyResult.port == null || proxyResult.host == null) {
  // Don't use the proxy
} else {
